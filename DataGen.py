...
def loadMidiData(cleanfiles):
    ...
    for c in cleanfiles:
        ...
        try:
            ...
            key_weight_distribution = [sum(semitone)/total_velocity for semitone in mid.get_chroma()]
...
def getKerasData(mididict, key_distributions):
    ...
    return data, binaryVecs, (earliestNoteTime, latestNoteTime, shortestNoteDuration, longestNoteDuration, lowestNotePitch, highestNotePitch)
    ...
    for i in range(len(data)):
        ...
        # only take first 1000 notes of song (for now)
         ...
         for j in range(len(data[i])):
            ...
            newNoteDuration = float(remap(note[1], timeScalars[2], timeScalars[3], 0, 1))
            # 0, 127 -> 0, 1
            newNoteVelocity = float(remap(note[2], 0, 127,0,1))
            # 0, 127 -> 0, 1
            newNotePitch = float(remap(note[3], timeScalars[4], timeScalars[5], 0,1))
            newLabels.append((newNoteTime, newNoteDuration, newNoteVelocity, newNotePitch))
            priorNotes = []
            for k in range(sequenceSize):
                # get previous notes of length sequenceSize
                prevNote = data[i][j - (k + 1)]
                pNoteTime = float(remap(prevNote[0], timeScalars[0], timeScalars[1], 0, 1))
                pNoteDuration = float(remap(prevNote[1], timeScalars[2], timeScalars[3], 0, 1))
...
def processKerasDataLastNote(data, timeScalars):
    ...
    for i in range(len(data)):
        ...
        for note in data[i]:
            ...
            newNotes.append((newNoteTime, newNoteDuration, newNoteVelocity, newNotePitch, lastNote[0],lastNote[1],lastNote[2],lastNote[3]))
